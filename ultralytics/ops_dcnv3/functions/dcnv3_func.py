...
def _get_reference_points(spatial_shapes, output_size, device, kernel_h, kernel_w, dilation_h, dilation_w, stride_h=1, stride_w=1):
...
def dcnv3_core_pytorch(
    ...
    # The shape of the `offset` tensor, as passed into this function,
    # is the GROUND TRUTH for the output shape. It has already been strided.
    _, H_out, W_out, _ = offset.shape

    # Use the corrected helper function, passing the ground truth output size.
    # This ensures `ref` has the same H_out, W_out as `offset`.
    ref = _get_reference_points(
        spatial_shapes=input.shape,
    ...
    # This operation will now succeed because `sampling_locations` and `offset`
    # are guaranteed to have the same H_out and W_out dimensions.
    ...
    # Use .reshape() for safety against non-contiguous tensors.
    mask = mask.reshape(N_, H_out*W_out, group, P_).transpose(1, 2).\
